...
/* ORIA brand film — corte 4:5 (1080×1350) para EMBED em seção de LP.
   ...
   (ExamCard, MarkerBar, Caption, Mark, inout). */

import React from "react";
import { Easing, clamp, useSprite, useTime } from "./animations";
import { C, FD, FB, inout, Mark, Caption, FileGlyph } from "./brand";
import { ExamCard, MarkerBar } from "./scenes";

export const MW = 1080,
  MH = 1350;

/* ── Fundo vivo, enquadrado para 4:5 ────────────────────────────────────── */
export function BackdropM() {
  const t = useTime();
  ...
  const offset = (n: number) => 1300 - (t * 0.15 + n * 0.4) * 1500;
  return (
...
export function JourneyTrackerM() {
  const t = useTime();
  const START = 9.4,
    END = 25.0;
  ...
  return (
    <div style={{ position: "absolute", left: 0, right: 0, bottom: 130, display: "flex", justifyContent: "center", opacity: op, pointerEvents: "none" }}>
  ...
  return (
    <div style={{ position: "absolute", inset: 0 }}>
      ...
      <Caption localTime={localTime} duration={duration}
        ...
        sub="Soltos em PDFs e fotos, cheios de números difíceis de entender." x={90} y={170} maxW={900} />
    </div>
  );
}

/* ── Cena 2 — você envia, no WhatsApp ───────────────────────────────────── */
export function SceneSendM() {
  const { localTime, duration } = useSprite();
  ...
  return (
    <div style={{ position: "absolute", inset: 0 }}>
      ...
      <Caption localTime={localTime} duration={duration}
        ...
        sub="Mande em PDF ou foto. Sem app, sem cadastro." x={90} y={170} maxW={900} />
    </div>
  );
}

/* ── Sistema orbital reutilizável (centro do ato "processa") ─────────────── */
    ...
    <div style={{ position: "absolute", left: cx, top: cy, transform: "translate(-50%,-50%)" }}>
      ...
      <div
        style={{
          ...
          marginTop: -halo / 2,
          borderRadius: 999,
          background: "radial-gradient(circle, rgba(159,230,189,0.22), transparent 70%)",
        }}
      />
      <div style={{ transform: `translate(-50%,-50%) scale(${enter * pulse})`, position: "absolute", left: "50%", top: "50%" }}>
        <Mark size={markSize} color={C.glow} />
      </div>
    </div>
  );
}
...
export function SceneCloseM() {
  ...
  return (
    <div style={{ position: "absolute", inset: 0 }}>
      ...
      <div style={{ position: "absolute", left: 100, right: 100, bottom: 150, textAlign: "center", opacity: discIn, fontFamily: FB, fontSize: 19, lineHeight: 1.5, color: "rgba(244,242,238,0.4)" }}>
